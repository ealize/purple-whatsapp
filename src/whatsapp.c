...
static gboolean whatsapp_send_packet(PurpleConnection* gc,guint16 packet_id,const gchar* paramstring , ...){
    gboolean res;
    guint32 intparam;
    guint8 byteparam;
...
const char *whatsapp_normalize_nick(const PurpleAccount *acct,
                               const char *input)
{
    if (input[0] == '[')
    {
...
void hon_parse_initiall_statuses(PurpleConnection *gc,gchar* buffer){
    ...
    while (count-- > 0)
    {
        ...
        if (status == WHATSAPP_STATUS_INLOBBY || status == WHATSAPP_STATUS_INGAME)
        {
            server = read_string(buffer);
            status_id = WHATSAPP_STATUS_INGAME_S;
    ...
    if (nick)
    {
        if (status == WHATSAPP_STATUS_INGAME)
...
void hon_parse_chat_entering(PurpleConnection *gc,gchar* buffer)
{
    ...
    while (count--)
    {
        ...
        extra = nickname;
        nickname = whatsapp_normalize_nick(gc->account,nickname);
...
void hon_parse_chat_join(PurpleConnection *gc,gchar* buffer){
    ...
    extra = nick;
    nick = whatsapp_normalize_nick(gc->account,nick);
    status = read_byte(buffer);
    flags = read_byte(buffer);
    flag = read_string(buffer);
    shield = read_string(buffer);
...
gboolean hon_send_pong(PurpleConnection *gc){
    return whatsapp_send_packet(gc,HON_CS_PONG/*0x02*/,"");
}
gboolean hon_send_login(PurpleConnection *gc, const gchar* cookie,\
    ...
    hon_account* hon = gc->proto_data;
    return whatsapp_send_packet(gc,HON_CS_AUTH_INFO/*0xFF*/,"isssiib",
    hon->self.account_id,hon->cookie,ip,auth_token,
    protocolVersion,(guint32)5,(char)0);
}
gboolean whatsapp_send_pm(PurpleConnection* gc,const gchar *username,const gchar* message){
    return whatsapp_send_packet(gc,HON_CS_PM/*0x1C*/,"ss",username,message);
}
gboolean hon_send_join_chat(PurpleConnection* gc,const gchar *room){
    return whatsapp_send_packet(gc,HON_CS_JOIN_CHANNEL/*0x1e*/,"s",room);
}
gboolean hon_send_join_chat_password(PurpleConnection* gc,const gchar *room,const gchar* password){
    return whatsapp_send_packet(gc,HON_CS_JOIN_CHANNEL_PASSWORD/*0x46*/,"ss",room,password);
}
gboolean hon_send_leave_chat(PurpleConnection* gc,gchar* name){
    return whatsapp_send_packet(gc,HON_CS_LEAVE_CHANNEL/*0x22*/,"s",name);
}
gboolean hon_send_chat_message(PurpleConnection *gc, guint32 id, const char *message){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_MSG/*0x03*/,"si",message,id);
}
gboolean hon_send_chat_topic(PurpleConnection *gc, guint32 id, const char *topic){
    return whatsapp_send_packet(gc,HON_CS_UPDATE_TOPIC/*0x30*/,"is",id,topic);
}
gboolean hon_send_whisper(PurpleConnection* gc,const gchar *username,const gchar* message){
    return whatsapp_send_packet(gc,HON_CS_WHISPER/*0x08*/,"ss",username,message);
}
gboolean hon_send_clan_invite(PurpleConnection* gc,const gchar *username){
    return whatsapp_send_packet(gc,HON_CS_CLAN_ADD_MEMBER/*0x47*/,"s",username);
}
gboolean hon_send_clan_message(PurpleConnection* gc,const gchar *message){
    return whatsapp_send_packet(gc,HON_CS_CLAN_MESSAGE/*0x13*/,"s",message);
}
gboolean hon_send_whois(PurpleConnection* gc,const gchar *username){
    return whatsapp_send_packet(gc,HON_CS_USER_INFO/*0x2a*/,"s",username);
}
gboolean hon_send_add_buddy_notification(PurpleConnection* gc,guint32 selfid, gchar* nick){
    return whatsapp_send_packet(gc,HON_CS_BUDDY_ADD_NOTIFY/*0x0d*/,"is",selfid,nick);
}
gboolean hon_send_channel_kick(PurpleConnection* gc,guint32 chatid, guint32 kickedid){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_KICK/*0x31*/,"ii",chatid,kickedid);
}
gboolean hon_send_channel_promote(PurpleConnection* gc,guint32 chatid, guint32 promotedid){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_PROMOTE/*0x3A*/,"ii",chatid,promotedid);
}
gboolean hon_send_channel_demote(PurpleConnection* gc,guint32 chatid, guint32 demotedid){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_DEMOTE/*0x3B*/,"ii",chatid,demotedid);
}
gboolean hon_send_channel_ban(PurpleConnection* gc,guint32 chatid, const gchar* banned){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_BAN/*0x32*/,"is",chatid,banned);
}
gboolean hon_send_channel_unban(PurpleConnection* gc,guint32 chatid, const gchar* banned){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_UNBAN/*0x33*/,"is",chatid,banned);
}
gboolean hon_send_channel_password(PurpleConnection* gc,guint32 chatid, const gchar* password){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_SET_PASSWORD/*0x33*/,"is",chatid,password);
}
gboolean hon_send_channel_silence(PurpleConnection* gc,guint32 chatid, const gchar* user,guint32 duration){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_SILENCE_USER/*0x38*/,"isi",chatid,user,duration);
}
gboolean hon_send_channel_auth_enable(PurpleConnection* gc,guint32 chatid){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_AUTH_ENABLE,"i",chatid);
}
gboolean hon_send_channel_auth_disable(PurpleConnection* gc,guint32 chatid){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_AUTH_DISABLE,"i",chatid);
}
gboolean hon_send_channel_auth_add(PurpleConnection* gc,guint32 chatid,gchar* username){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_AUTH_ADD,"is",chatid,username);
}
gboolean hon_send_channel_auth_delete(PurpleConnection* gc,guint32 chatid,gchar* username){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_AUTH_DELETE,"is",chatid,username);
}
gboolean hon_send_channel_auth_list(PurpleConnection* gc,guint32 chatid){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_AUTH_LIST,"i",chatid);
}
gboolean hon_send_emote(PurpleConnection* gc,guint32 chatid,const gchar* string){
    return whatsapp_send_packet(gc,HON_CS_CHANNEL_EMOTE,"si",string,chatid);
}
gboolean whatsapp_send_join_game(PurpleConnection* gc,const gchar* status,guint32 matchid,gchar* server){
    return whatsapp_send_packet(gc,WHATSAPP_CS_JOIN_GAME,"sis",status,matchid,server);
}
gboolean hon_send_whisper_buddies(PurpleConnection* gc,const gchar* message){
    return whatsapp_send_packet(gc,HON_CS_WHISPER_BUDDIES,"s",message);
}
gboolean hon_send_accept_buddy(PurpleConnection* gc,const gchar* buddy){
    return whatsapp_send_packet(gc,HON_CS_BUDDY_ACCEPT,"s",buddy);
}
