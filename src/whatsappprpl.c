...
static void whatsappprpl_cleanup_blist(PurpleConnection* gc){
...
static void whatsappprpl_update_buddies(PurpleConnection* gc){
...
static void whatsappprpl_update_clanmates(PurpleConnection* gc){
 	hon_account* hon = gc->proto_data;
 ...
 * prpl functions
 */
static const char *whatsappprpl_list_icon(PurpleAccount *acct, PurpleBuddy *buddy)
{
	...
	if (status == WHATSAPP_STATUS_INGAME || status == WHATSAPP_STATUS_INLOBBY)
	{
		purple_debug_info(HON_DEBUG_PREFIX, "%s icon: %s\n",
		buddy->name,WHATSAPP_INGAME_EMBLEM);
		return WHATSAPP_INGAME_EMBLEM;
	}
...
static char *whatsappprpl_status_text(PurpleBuddy *buddy) {
...
static const char* whatsappprpl_list_emblem(PurpleBuddy *b)
{
	...
	{
		return WHATSAPP_PREMIUM_EMBLEM;
	}
...
static void whatsappprpl_tooltip_text(PurpleBuddy *buddy,
...
static GList *whatsappprpl_status_types(PurpleAccount *acct)
{
	GList *types = NULL;
	...
	purple_debug_info(HON_DEBUG_PREFIX, "returning status types for %s: %s, %s,%s\n",
		acct->username,
		HON_STATUS_ONLINE_S, WHATSAPP_STATUS_INGAME_S,HON_STATUS_OFFLINE_S);
	...
	type = purple_status_type_new_with_attrs(PURPLE_STATUS_UNAVAILABLE,
		WHATSAPP_STATUS_INGAME_S, NULL, TRUE, FALSE, FALSE,
...
static GList *whatsappprpl_blist_node_menu(PurpleBlistNode *node) {
	return NULL;
...
static GList *whatsappprpl_chat_info(PurpleConnection *gc) {
...
static GHashTable *whatsappprpl_chat_info_defaults(PurpleConnection *gc,
											  const char *room) 
{
	GHashTable *defaults;
...
static void whatsappprpl_login_callback(gpointer data, gint source, const gchar *error_message)
{
    PurpleConnection *gc = data;
	...
	if(!(url_text)){
		...
		if (account_data->type != PHP_ARRAY)
		{
			...
			if (!res || !res->u.int_val)
			{
				...
				whatsappprpl_cleanup_blist(gc);
				whatsappprpl_update_buddies(gc);
				whatsappprpl_update_clanmates(gc);
				...
				purple_connection_update_progress(gc, _("Connecting"),
 				...
 				if (purple_proxy_connect(gc, gc->account, 
 				                         ...
 				                         HON_CHAT_PORT,
 					whatsappprpl_login_callback, gc) == NULL)
				{
					purple_connection_error_reason (gc,
						PURPLE_CONNECTION_ERROR_NETWORK_ERROR,
						_("Unable to connect"));
...
static void whatsappprpl_login(PurpleAccount *acct)
{
...
static void whatsappprpl_close(PurpleConnection *gc)
{
	hon_account* hon = gc->proto_data;
	close(hon->fd);
...
static int whatsappprpl_send_im(PurpleConnection *gc, const char *who,
						   const char *message, PurpleMessageFlags flags)
{
	...
	purple_debug_info(HON_DEBUG_PREFIX, "sending message to %s: %s\n",
		who, message);
#endif
	res = whatsapp_send_pm(gc,who,plain);
	g_free(plain);
	return res;
}
...
static void whatsappprpl_get_info(PurpleConnection *gc, const char *username) {
...
static void whatsappprpl_add_buddy(PurpleConnection *gc, PurpleBuddy *buddy,
							  PurpleGroup *group)
{
	honprpl_nick2id(buddy,honpurple_add_buddy_nick2id_cb,honpurple_add_buddy_nick2id_error_cb);
}
static void whatsappprpl_add_buddies(PurpleConnection *gc, GList *buddies,
								GList *groups)
{
	GList *buddy = buddies;
	GList *group = groups;
	...
	while (buddy && group) {
		whatsappprpl_add_buddy(gc, (PurpleBuddy *)buddy->data, (PurpleGroup *)group->data);
...
static void whatsappprpl_remove_buddy(PurpleConnection *gc, PurpleBuddy *buddy,
								 PurpleGroup *group)
{
	guint32 buddy_id;
...
static void whatsappprpl_remove_buddies(PurpleConnection *gc, GList *buddies,
								   GList *groups) 
{
	GList *buddy = buddies;
	GList *group = groups;
	...
	while (buddy && group) {
		whatsappprpl_remove_buddy(gc, (PurpleBuddy *)buddy->data,
...
static void whatsappprpl_join_chat(PurpleConnection *gc, GHashTable *components) {
...
static char *whatsappprpl_get_chat_name(GHashTable *components) {
...
static void whatsappprpl_chat_leave(PurpleConnection *gc, int id) {
	PurpleConversation* conv = purple_find_chat(gc,id);
	hon_send_leave_chat(gc,conv->name);
}


static int whatsappprpl_chat_send(PurpleConnection *gc, int id, const char *message,
...
static void whatsappprpl_set_chat_topic(PurpleConnection *gc, int id,
								   const char *topic)
{
	hon_send_chat_topic(gc,id,topic);
}
static PurpleCmdRet whatsappprpl_send_whisper(PurpleConversation *conv, const gchar *cmd,
										   ...
										   gchar **args, gchar **error, void *userdata) 
{
	...
	if (!g_strcmp0(command,"add"))
	{
		...
		whatsappprpl_add_buddy(gc,buddy,NULL);
	}
	else if (!g_strcmp0(command,"del"))
	{
		...
		whatsappprpl_remove_buddy(gc,buddy,NULL);
	}
	else {
								...
								gchar **args, gchar **error, void *userdata)
{
	...
	whatsappprpl_join_chat(conv->account->gc, table);
...
{
	PurpleConvChat* chat = PURPLE_CONV_CHAT(conv);
	whatsappprpl_chat_leave(conv->account->gc, chat->id);
	return PURPLE_CMD_RET_OK;
}
...
{
	PurpleConvChat* chat = PURPLE_CONV_CHAT(conv);
	whatsappprpl_set_chat_topic(conv->account->gc,chat->id,args[0]);
	return PURPLE_CMD_RET_OK;
}
...
static PurpleCmdRet whatsappprpl_join_game(PurpleConversation *conv, const gchar *cmd,
										  gchar **args, gchar **error, void *userdata) 
{
	whatsapp_send_join_game(conv->account->gc,args[2],atoi(args[1]),args[0]);
	return PURPLE_CMD_RET_OK;
}
...
static PurplePluginProtocolInfo prpl_info =
{
	...
	NULL,               /* user_splits, initialized in whatsappprpl_init() */
	NULL,               /* protocol_options, initialized in whatsappprpl_init() */
	...
	whatsappprpl_list_icon,                  /* list_icon */
	whatsappprpl_list_emblem,                                /* list_emblem */
	whatsappprpl_status_text,                /* status_text */
	whatsappprpl_tooltip_text,               /* tooltip_text */
	whatsappprpl_status_types,               /* status_types */
	whatsappprpl_blist_node_menu,            /* blist_node_menu */
	whatsappprpl_chat_info,                  /* chat_info */
	whatsappprpl_chat_info_defaults,         /* chat_info_defaults */
	whatsappprpl_login,                      /* login */
	whatsappprpl_close,                      /* close */
	whatsappprpl_send_im,                    /* send_im */
	NULL,                   /* set_info */
	NULL,                /* send_typing */
	whatsappprpl_get_info,                   /* get_info */
	NULL,                 /* set_status */
	NULL,                   /* set_idle */
	NULL,              /* change_passwd */
	whatsappprpl_add_buddy,                  /* add_buddy */
	whatsappprpl_add_buddies,                /* add_buddies */
	whatsappprpl_remove_buddy,               /* remove_buddy */
	whatsappprpl_remove_buddies,             /* remove_buddies */
	NULL,                 /* add_permit */
	NULL,                   /* add_deny */
	NULL,                 /* rem_permit */
	NULL,                   /* rem_deny */
	NULL,            /* set_permit_deny */
	whatsappprpl_join_chat,                  /* join_chat */
	NULL,                /* reject_chat */
	whatsappprpl_get_chat_name,              /* get_chat_name */
	NULL,                /* chat_invite */
	whatsappprpl_chat_leave,                 /* chat_leave */
	NULL,               /* chat_whisper */
	whatsappprpl_chat_send,                  /* chat_send */
	NULL,                                /* keepalive */
	NULL,              /* register_user */
	NULL,                /* get_cb_info */
	NULL,                                /* get_cb_away */
	...
	NULL,                                /* buddy_free */
	NULL,               /* convo_closed */
	whatsapp_normalize_nick,                  /* normalize */
	NULL,             /* set_buddy_icon */
	NULL,               /* remove_group */
	NULL,                                /* get_cb_real_name */
	whatsappprpl_set_chat_topic,             /* set_chat_topic */
...
static PurplePluginInfo info =
{
	...
	N_("WhatsApp Protocol Plugin"),                              /* summary */
	N_("Protocol Plugin for WhatsApp chat"),                              /* description */
	NULL,                                                    /* author */
	"http://whatsapp.com/",                                          /* homepage */
	NULL,                                                    /* load */
	NULL,                                                    /* unload */
...
PURPLE_INIT_PLUGIN(honprpl, whatsappprpl_init, info);
